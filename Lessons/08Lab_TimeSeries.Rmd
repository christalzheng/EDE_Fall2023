...
2. Explore the components of times series: trend, seasonal, random (cyclical)
...
Temp.ts <- ts(BR_complete$Temperature_C, start=c(1905,1),frequency = 12)
...
temp_decomp <- stl(Temp.ts,s.window = "periodic") 
plot(temp_decomp) # seasonlality as important as trend, trend rise more recent 30 yrs
rain_decomp <- stl(Rain.ts,s.window = "periodic")
plot(rain_decomp) # the bigger grey square, the smaller the impact -> seasonality more impact than trend
...
temp.comp <- as.data.frame(temp_decomp$time.series[,1:3])
rain.comp <- as.data.frame(rain_decomp$time.series[,1:3])

temp.comp <- mutate(temp.comp,
        Observed = Temp_BR_processed$Temperature_C,     
        Date = Temp_BR_processed$Date)

rain.comp <- mutate(rain.comp,
...
temp_trend <- Kendall::SeasonalMannKendall(Temp.ts)
rain_trend <- Kendall::SeasonalMannKendall(Rain.ts)
