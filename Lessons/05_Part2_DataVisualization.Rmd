...
```{r theme, fig.height = 3, fig.width = 4, warning = FALSE}
...
```{r my theme, fig.height = 3, fig.width = 4, warning = FALSE}
...
```{r add line and text/ylab/axis setting, fig.height = 3, fig.width = 4, warning = FALSE}
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, size = 2.5) + # alpha-transperencey
  ...
  scale_color_viridis(option = "magma", direction = -1) + # change color
...
# place your additional edits here
  scale_y_continuous(expand = c(0, 0)) + # move figure closer to x axis
...
```{r multiple plots, fig.height = 7, fig.width = 6}
#install.packages("cowplot")
library(cowplot)
plot_grid(NvsP2, Nutrientplot, nrow = 2, align = 'h', rel_heights = c(1.25, 1)) # first need 1.25 space
