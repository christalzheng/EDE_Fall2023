...
Question: How would you report the results of this test (overall findings and report of statistical output)?
 r^2 explains r^2 percent of variability in irradiance
 p value < confidence level -> statistically significant and meaningful regression
>  
...
symmetrical around red line and flat red line
...
```{r log transform and remove outlier, fig.height = 3, fig.width = 4}
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + # y axis is log transformed
  geom_point() 
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients) # corr matrix
corrplot(PeterPaulCorr, method = "ellipse") # thinner the ellipse, higher corr
corrplot.mixed(PeterPaulCorr, upper = "ellipse")
```

## AIC to select variables, over-parameterize risk with additional variables
