...
Ozone.density <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  ...
  labs(x="Ozone AOI value", y="Density", fill="Year")
print(Ozone.density)

Ozone.boxplot <- ggplot(EPAair, aes(y = Ozone, x = as.factor(Year))) +
  geom_boxplot()+
  stat_summary(geom = "text",fun = max, vjust = -1, size = 4, label = c("a","b"))+
  labs(x="Year", y="Ozone AOI value")+
  ylim(0,150)
print(Ozone.boxplot)

Ozone.violin <- ggplot(EPAair, aes(y = Ozone, x = as.factor(Year))) +
...
EPACorr <- cor(EPAair.corr) # corr matrix
...
TPAIC <- lm(data = EPAair, PM2.5 ~ Ozone + mean_Lat + 
              mean_Lng + Month + Year)
...
summary(lm(data = EPAair, PM2.5 ~ Ozone + mean_Lat + mean_Lng + Month))
```
