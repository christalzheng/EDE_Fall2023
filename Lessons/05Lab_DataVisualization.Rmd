...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    # don't need 'mu', -> can only use paste)
    )
...
```{r fill missing data using factor}
...
factor(PeterPaul.chem.nutrients$month, level =1:12, labels=month.abb)

#Try again: Plot how many Lake observations occur each month
ggplot(PeterPaul.chem.nutrients,aes(x=factor(PeterPaul.chem.nutrients$month, level =1:12, labels=month.abb)
))+
  geom_bar() +
...
```{r exersice, fig.height = 3, fig.width = 4, warning = FALSE}

# 1. 
# Plot surface temperatures by day of  year.  # depth -> 0
plot1 <- ggplot(filter(PeterPaul.chem.nutrients,depth==0),aes(x=daynum, y= temperature_C, color = year4))+
  geom_point()+
  facet_wrap(vars(lakename), nrow = 2)+
  ylab(paste("surface temperatures (C)"))+ # labs(y="temperature")
  theme_gray()
print(plot1)
...
```{r exercise, fig.height = 3, fig.width = 4, warning = FALSE}
...
plot3 <- ggplot(EPAair, aes(x=PM2.5,y=Ozone,color=mean_Lat))+
