...
#Get the data, which starts on line 30
gage_data <- read.table(theURL,skip = 2188,header=TRUE,sep='\t',stringsAsFactors = T) 
...
df.energy <-data.frame("Year" = rep(2022,52),
                       "states" = the_states,
                        "Average_Retail_price" = as.numeric(the_price),
                         "Net_summer_capacity" = as.numeric(gsub(',','',the_capacity)),
...
#3 Construct a dataframe from the values
df.energy <-data.frame("Year" = rep(the_year,52),
                       "states" = the_states,
                        "Average_Retail_price" = as.numeric(the_price),
                         "Net_summer_capacity" = as.numeric(gsub(',','',the_capacity)),
                       "Net_generation" = as.numeric(gsub(',','',net_generation)),
...
df_years <- lapply(seq(2017,2022),scrape.it) %>% bind_rows()
