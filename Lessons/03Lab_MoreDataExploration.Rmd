...
Tip: remove from environment: rm()
...
> Answer: in environment: list and matrix show as data, vector show as values
...
```{r DataExercise-1a}
H <- c(83,83,83,78,71,79,82,85,84,82)
L <- c(56,58,60,61,63,63,64,64,63,60)
```
...
```{r DataExercise-1c}
Temperature <- data.frame(H_c,L_c)
names(Temperature) <- c("High Temperature","Low Temperature")
Temperature
```
...
```{r DataExercise-1d}
summary(Temperature)
summary(Temperature$`High Temperature`)
sd(Temperature$`High Temperature`)
