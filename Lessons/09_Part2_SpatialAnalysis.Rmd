...
upper_sf<- st_read(here('Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == 'Upper Neuse')


#Create a map
mapview(huc8_sf,col.regions='orange',layer.name='HUC 8')+
  mapview(upper_sf,col.regions='purple')

ggplot()+
  geom_sf(data=huc8_sf,fill='orange')+
  geom_sf(data=upper_sf,fill='purple')
...
mapview(counties_sf_join, 
  ...
  mapview(epa_pm25_sites_sf, cex = 'maxPM') #size
...
the_huc <- huc8_sf_utm[the_triangle,]
mapview(the_huc)+mapview(the_triangle,alpha.regions = 0, color = 'red')
...
result30 <- st_intersection(counties_sf_utm,the_site_counties,sparse = FALSE)

sel30 <- counties_sf_utm[result30,]
mapview(sel30)
...
ggplot()  +
  ...
  geom_sf(data = state_sf_utm, color='red',size=2) +  # mask all
