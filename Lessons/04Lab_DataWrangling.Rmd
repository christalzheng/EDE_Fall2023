...
NTL.phys.data.year <- mutate(NTL.phys.data, year = year(sampledate)) 
Year_1999 <- filter(NTL.phys.data.year, year == 1999)
# Year_1999 <- filter(NTL.phys.data, year4 == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
Year_1990_1999 <- filter(NTL.phys.data.year, year %in% c(1990:1999) & lakename == "Tuesday Lake")
#NTL.phys.data.year <- mutate(NTL.phys.data.year, weekday = wday(sampledate)) 
#Year_1990_1999_Tue <- filter(NTL.phys.data.year, weekday == 2)
...
NTL.phys.data.processed_year <- 
  NTL.phys.data %>%
  #filter(year4 %in% c(1990,1999)) %>%
  #mutate(weekday = wday(sampledate), month = month(sampledate)) %>%
  #filter(weekday == 2) %>%
  #filter(month == 7)
  filter (year4 %in% c(1990,1999) & lakename == "Tuesday Lake" & month(sampledate) == 7)
...
NTL.phys.data.gathered <- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck # cols = c(irradianceWater,irradianceDeck)
                                       , names_to = "Irradiance_Type", values_to = "Irradiance_Value")
